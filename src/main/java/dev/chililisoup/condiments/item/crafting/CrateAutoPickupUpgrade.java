...
public class CrateAutoPickupUpgrade extends CustomRecipe {
    //? if < 1.21 {
    /*public CrateAutoPickupUpgrade(ResourceLocation id, CraftingBookCategory category) { super(id, category); }
    *///?} else
    public CrateAutoPickupUpgrade(CraftingBookCategory category) { super(category); }
        ...
        for (int k = 0; k < VersionHelper.size(input); ++k) {
            ...
            if (!itemStack.isEmpty()) {
                if (itemStack.getItem() instanceof CrateItem) ++i;
                    ...
                    if (!(itemStack.is(ModItemTags.CRATE_AUTO_PICKUP_UPGRADE_ITEMS))) return false;
                    ++j;
                }
    ...
    *///?} else
    ...
    public @NotNull RecipeSerializer<?> getSerializer() {
        return ModRecipeSerializers.CRATE_AUTO_PICKUP_UPGRADE.get();
    }
}
